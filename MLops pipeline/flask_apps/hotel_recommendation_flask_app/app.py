...
# -----------------------------
# 2. Create Flask app and artifact paths
# -----------------------------
...
def build_hotel_summary(hotels_df):
    # -----------------------------
    # 3. Build hotel-level summary table
    # -----------------------------
...
def get_popular_hotels(model_bundle, hotel_summary_df):
    # -----------------------------
    # 4. Prepare popular hotel ranking
    # -----------------------------
...
def get_similar_hotels(model_bundle, hotel_summary_df, selected_hotel, top_n):
    # -----------------------------
    # 5. Recommend hotels similar to one hotel
    # -----------------------------
...
def get_user_recommendations(model_bundle, hotel_summary_df, selected_user, top_n):
    # -----------------------------
    # 6. Recommend hotels for one user
    # -----------------------------
...
def home():
    ...
    # Defaults keep the browser page usable on the first GET request.
    ...
    # Request values can come from the initial page, form submit, or query string.
    ...
    if selected_hotel not in hotel_options:
        # Fall back to a known hotel instead of rendering a broken selection.
        selected_hotel = default_hotel

    if selected_user not in user_options:
        # Fall back to a known user for the same reason.
        selected_user = default_user

    # Prepare all three recommendation views before rendering the template.
    ...
    # The template uses this to show context for the selected hotel.
    ...
    return render_template(
        ...
        dataset_file=display_path(HOTELS_DATA_PATH),
...
def popular_hotels():
    # -----------------------------
    # 11. Popular hotels API endpoint
    # -----------------------------
    ...
    # Slice the cached popularity table instead of recalculating rankings for every request.
...
def similar_hotels():
    # -----------------------------
    # 12. Similar hotels API endpoint
    # -----------------------------
    ...
    # read_request_data lets this endpoint work with JSON, form data, or query params.
    selected_hotel = str(payload.get("hotel_name", "")).strip()

    top_n = read_positive_int(payload.get("top_n"), 5)

    if not selected_hotel:
        # Missing input is a client error, so return 400 with a clear message.
    ...
    if similar_hotels_df.empty and selected_hotel not in assets["model_bundle"]["hotel_index"].index:
        # Unknown hotel names should not return an empty success response.
...
def user_recommendations():
    # -----------------------------
    # 13. User recommendations API endpoint
    # -----------------------------
    ...
    # The API uses user_code because the recommender was built from user-item history.
    selected_user = str(payload.get("user_code", "")).strip()

    top_n = read_positive_int(payload.get("top_n"), 5)

    if not selected_user:
        # Missing user code is a request problem, not a server failure.
    ...
    if selected_user not in assets["user_item_matrix_df"].index:
        # Return 404 when the user is not part of the saved recommender matrix.
