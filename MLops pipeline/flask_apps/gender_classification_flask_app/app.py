...
# -----------------------------
app = Flask(__name__, template_folder="templates")  # create the Flask service used by browser and API clients.
register_error_handlers(app, "Gender Classification Flask API")  # reuse shared JSON error responses.
LOGGER = logging.getLogger("voyage.gender_api")  # keep prediction logs under a service-specific logger.
...
# -----------------------------
# 2. Convert raw classifier score
# -----------------------------
...
# -----------------------------
# 4. Render browser prediction page
# -----------------------------
def home():
    ...
    return render_template(
        ...
        dataset_file=display_path(USERS_DATA_PATH),
...
def predict():
    ...
    # Accept the same name field from JSON, form data, or query parameters.
    person_name = str(payload.get("name", "")).strip()

    if not person_name:
        # Missing input is handled as a client error with a clear message.
        return jsonify({"error": "Please provide a name value."}), 400

    # Keep the predicted label and raw margin together so the response is explainable.
...
if __name__ == "__main__":
    # -----------------------------
    # 10. Local development entry point
    # -----------------------------
