...
# -----------------------------
st.set_page_config(page_title="Gender Classification", layout="wide")  # use wide layout for form, result card, and dataset panels.

# This dashboard reads one saved classifier artifact and turns it into a browser demo.
...
# -----------------------------
# 6. Convert raw classifier score
# -----------------------------
...
# The dataset supports dashboard metrics, sample names, and distribution views.
...
# Count all rows shown in the dashboard, including unlabeled records.
total_records = len(users_df)

# Count only rows that can directly support supervised classification.
labeled_records = len(labeled_users_df)

# "none" values are kept visible because they explain why not every row is a training label.
unlabeled_records = int((users_df["gender"] == "none").sum())

# Company count gives a quick source-data diversity check for the viewer.
company_count = users_df["company"].nunique()

# These tables drive the chart and dataframe in the dataset overview tabs.
...
# -----------------------------
# 10. Render hero and summary metrics
# -----------------------------
st.markdown(
    """
    <div class="hero-card">
...
# The top metrics explain the dataset before the user tests a prediction.
...
with input_col:
    ...
    with st.form("gender_prediction_form", border=False):
        # The typed name path is useful for manual testing during recording.
...
# Use the selected sample name only when the user chooses it from the dropdown.
selected_name = person_name if sample_name == "Use typed name" else sample_name

# Trim spaces so empty-looking inputs do not get sent to the model.
selected_name = selected_name.strip()

# -----------------------------
# 12. Run prediction after submit
# -----------------------------
if submitted and selected_name:
    ...
    # The raw decision score is later converted into a simple strength label for display.
...
with result_col:
    ...
    if prediction_result:
        # Render the latest prediction from session state so it stays visible after reruns.
        st.markdown(
            f"""
        ...
        # The raw score is useful for explaining that the model has an internal margin.
        ...
        # Before the first submit, keep the result area ready instead of leaving it blank.
        st.markdown(
            """
            <div class="result-card">
                <div class="result-label">Predicted gender</div>
...
# -----------------------------
# 14. Render dataset tabs
# -----------------------------
overview_tab, data_tab = st.tabs(["Dataset Overview", "Dataset Preview"])  # split charts and raw preview into simple sections.
...
with overview_tab:
    ...
    with overview_col_2:
        # Company distribution is shown as a table because it can contain more categories than a small chart.
        st.markdown(
            """
            <div class="panel-card">
...
with data_tab:
    ...
    # Limit preview size so the page stays readable while recording.
