...
# -----------------------------
st.set_page_config(page_title="Hotel Recommender", layout="wide")  # use wide layout for recommendation tables and charts.

# This app is artifact-driven: it reads a saved recommender instead of retraining anything.
...
@st.cache_data
# -----------------------------
# 2. Load saved recommender assets
# -----------------------------
def load_assets():
...
# -----------------------------
# 5. Build similar hotel recommendations
# -----------------------------
...
# -----------------------------
# 6. Build personalized user recommendations
# -----------------------------
...
# The summary table gives every recommendation row readable hotel details.
hotel_summary_df = build_hotel_summary(hotels_df)

# Popular picks come from the saved artifact, then get enriched with summary fields.
popular_hotels_df = get_popular_hotels(model_bundle, hotel_summary_df)

# Evaluation scores are stored inside the artifact so the dashboard can show model comparison.
evaluation_results_df = model_bundle["evaluation_results"].copy()

# The user-item matrix is the base table for personalized recommendations.
user_item_matrix_df = model_bundle["user_item_matrix"]

user_seen_counts = (user_item_matrix_df > 0).sum(axis=1)

# Keep only users who still have unseen hotels left to recommend.
eligible_users = user_seen_counts[user_seen_counts < user_item_matrix_df.shape[1]].index.tolist()

# Destination-level counts help explain where most bookings happened.
top_places_df = (
...
with hero_col_1:
    # Keep the objective visible before the user starts switching between tabs.
    st.markdown(
        """
...
with hero_col_2:
    # Surface the best saved model metric as a quick checkpoint for the demo.
...
with st.expander("Technical Notes", expanded=False):
    # Keep artifact and compatibility notes available without crowding the main dashboard.
    st.write(
...
with overview_tab:
    ...
    # These cards summarize the recommender artifact before showing individual outputs.
...
with similar_tab:
    ...
    with similar_control_col:
        # The selected hotel becomes the anchor item for item-to-item recommendation.
        ...
        # Show basic hotel context next to the selector so the recommendation has meaning.
...
# -----------------------------
# 14. Render personalized recommendations tab
# -----------------------------
with user_tab:
    ...
    # Prefer a user with unseen hotels so the demo can produce recommendations.
    ...
    with user_filter_col_1:
        # User code controls the interaction history used by collaborative filtering.
    ...
    if user_recommendations_df.empty:
        # This can happen when the selected user has already interacted with every hotel.
        st.info(
...
# -----------------------------
# 15. Render model evaluation tab
# -----------------------------
with metrics_tab:
