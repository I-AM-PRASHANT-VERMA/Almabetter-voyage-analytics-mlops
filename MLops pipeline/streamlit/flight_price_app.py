...
# Streamlit uses the same project root and artifacts as the Flask API so both stay consistent.
...
# -----------------------------
# 1. Configure Streamlit page and artifact paths
# -----------------------------
st.set_page_config(page_title="Flight Price Prediction", layout="wide")  # use a wide layout for dashboard-style content.
...
DEFAULT_FLIGHTS_DATA_PATH = (
    BASE_DIR / "dataset" / "travel_capstone" / "flights.csv"
    if (BASE_DIR / "dataset" / "travel_capstone" / "flights.csv").exists()
    else REPO_ROOT / "dataset" / "travel_capstone" / "flights.csv"
)
FLIGHTS_DATA_PATH = Path(os.getenv("FLIGHT_DATASET_PATH", DEFAULT_FLIGHTS_DATA_PATH))
...
# The saved pipeline contains its own encoder, so the UI must send raw columns.
...
def predict_with_api(
    ...
    # Send the same raw feature names that the trained pipeline expects.
    payload = {
...
def build_prediction_input(
    ...
    input_row = {  # keep the form values in the same raw feature format used during training.
        "time": float(travel_time),  # flight duration selected in the slider.
        "year": int(travel_date.year),  # year extracted from the selected date.
        "month": int(travel_date.month),  # month extracted from the selected date.
        "day": int(travel_date.day),  # day extracted from the selected date.
        "from": departure_city,  # origin city kept as raw category for the saved encoder.
        "to": arrival_city,  # destination city kept as raw category for the saved encoder.
        "flightType": flight_type,  # cabin type kept as raw category for the saved encoder.
        "agency": agency,  # agency kept as raw category for the saved encoder.
    }

    input_df = pd.DataFrame([input_row], columns=MODEL_FEATURE_COLUMNS)  # build one prediction row in the expected order.

    return input_df


# -----------------------------
# 11. Build route summary table
# -----------------------------
...
with st.sidebar:
    ...
    if use_flight_api():
        ...
        run_name = api_model_info.get("run_name") or api_model_info.get("mlflow_run_name")
        if run_name:
            st.caption(f"Training run: {run_name}")
    else:
...
# -----------------------------
# 14. Render hero and dashboard metrics
# -----------------------------
...
# -----------------------------
# 16. Run prediction after submit
# -----------------------------
# Prediction happens only after submit so normal widget reruns do not keep firing the model.
if submitted:
    try:
...
# -----------------------------
# 18. Render insight and dataset tabs
# -----------------------------
# Tabs separate market-level analysis from raw dataset preview.
